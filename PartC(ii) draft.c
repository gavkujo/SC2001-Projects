...
int main() {
    FILE *file = fopen("key_comparisons_vs_S.csv", "w");
    if (file == NULL) {
        printf("Error opening file!\n");
        return 1;
    }
    fprintf(file, "Threshold_S,KeyComparisons\n");  // Header for CSV file

    int fixedSize = 10000;  // Example size for fixed n
    for (int s = 1; s <= 50; s += 5) {  // Varying S
        long long avgComparisons = performSortingAndRecord(fixedSize, s);
        if (avgComparisons >= 0) {  // Only write to file if no error occurred
            fprintf(file, "%d,%lld\n", s, avgComparisons);
        } else {
            printf("Error occurred with threshold S %d\n", s);
            break;
        }
    }
